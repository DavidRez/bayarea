...
export default {
  ...
  methods: {
    ...
    getDimensions () {
      const height = this.$refs.locations.clientHeight
    ...
    addMapMarker (lat, lng, color, pos, title) {
      ...
      const marker = new google.maps.Marker({
        ...
        map: this.map,
        zIndex: this.activeLocation === pos ? 4 : 2
      })
      if (this.global.locations.length > 1) {
    ...
    changeLocation (target) {
      const prevActive = this.activeLocation
